...
class NoiseRecordService:
    ...
    def is_location_covered(
        ...
        user_id = payload["id"]
        for rec in db.query(NoiseRecord).all():
            if haversine(lat, lon, rec.latitude, rec.longitude) <= radius_m:
                return True
        payload = {
            "type": "noise",
            "message": f"Inregistreaza nivelul poluarii fonice!.",
            "user": str(user_id)
        }
